...
export class AuthService {
  ...
  async refreshTokens(userId: string, refreshToken: string, ip: string): Promise<{ accessToken: string; expiresIn: number }> {
  ...
  async changePassword(userId: string, dto: ChangePasswordDto, ip: string): Promise<void> {
    ...
    const hashed = await bcrypt.hash(dto.newPassword, this.config.get<number>('jwt.bcryptRounds')!);

    await this.prisma.user.update({ where: { id: userId }, data: { password: hashed } });
    await this.logoutAll(userId);

    await this.logs.log({ userId, action: 'CHANGE_PASSWORD', resource: 'auth', ip });
  }
  ...
  async resetPassword(
    ...
    if (!requesterRoles.includes(RoleType.SUPER_ADMIN)) {
      ...
      const allowedTargets: string[] = [RoleType.MATRIZADOR, RoleType.ARCHIVADOR];
      const canReset = targetRoles.every((r) => allowedTargets.includes(r));
      if (!canReset) {
        throw new ForbiddenException('Solo puede resetear contraseñas de Matrizadores y Archivadores');
      }
    }

    const tempPassword = dto.newPassword || this.generateTemporaryPassword();
    const hashed = await bcrypt.hash(tempPassword, this.config.get<number>('jwt.bcryptRounds')!);
  ...
  private generateAccessToken(userId: string, email: string, roles: string[], permissions: string[]): string {
